...
class as_kardex_productos_excel(models.AbstractModel):
    _name = 'report.as_warehouse.kardex_productos_report_xls.xlsx'
        ...
        for almacen in dict_almacen:
            ...
            for producto in product_categories:
                ...
                for line in all_movimientos_almacen:
                    ...
                    fecham = datetime.strptime(str(line[2]), '%Y-%m-%d') 
                    fechaw = datetime.strptime(data['form']['start_date'], '%Y-%m-%d')
                    if fecham < fechaw:
                        if line[4]>0:
