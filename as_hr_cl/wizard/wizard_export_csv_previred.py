...
class WizardExportCsvPrevired(models.TransientModel):
    ...
    date_from = fields.Date('Fecha Inicial', default=lambda self: time.strftime('%Y-%m-01'))
    date_to = fields.Date('Fecha Final', default=lambda self: time.strftime('%Y-%m-01'))
    ...
    def get_dias_trabajados(self, payslip):
        worked_days = 0
        if payslip:
            worked_days = payslip.contract_id.resource_calendar_id.as_total_days
    ...
    def get_tipo_linea(self, payslip):
        codigo = '00'
        if len(payslip.movimientos_personal) > 1:
            codigo = '01'
        return codigo

    @api.model
    def get_movimiento_personal(self, payslip):
        codigo = '00'
        if payslip.movimientos_personal != []:
            codigo = str(payslip.movimientos_personal[0].codigo)
        return codigo
    ...
    def action_generate_csv(self):
        ...
        payslip_recs = payslip_model.search([('date_from','=',self.date_from),('employer_id','=',self.employer_id.id),
                                             ])
        ...
        try:
            rut_emp, rut_emp_dv = self.employer_id.rut.split("-")
        ...
        for payslip in payslip_recs:
            ...
            line_employee = [self._acortar_str(rut, 11), 
                             ...
                             #15
                             self.get_movimiento_personal(payslip and payslip[0] or False),
                             ...
                             payslip.date_from.strftime("%d/%m/%Y") if self.get_movimiento_personal(payslip and payslip[0] or False) != '0' else '00/00/0000', 
                             #payslip.date_from if payslip.date_from else '00/00/0000', 
                             #17 Fecha fin movimiento personal (dia-mes-año)
                             payslip.date_to.strftime("%d/%m/%Y") if self.get_movimiento_personal(payslip and payslip[0] or False) != '0' else '00/00/0000', 
                             #Si declara mov. personal 1, 3, 4, 5, 6, 7, 8 y 11 Fecha Desde
                             #es obligatoria y debe estar dentro del periodo de remun
...
#caso se paga en la mutual respectiva. Datos no numérico 
                             ...
                             payslip.employer_id.ccaf_id.codigo if payslip.employer_id.ccaf_id.codigo else "00",
                             ...
                             payslip.employer_id.mutualidad_id.codigo if payslip.employer_id.mutualidad_id.codigo else "00",
        ...
        filename = "Previred_%s.txt" % (self.date_to)
        id_file = self.env['txt.extended'].create({'file_name': filename,'txt_file': base64.encodebytes(output.getvalue().encode())})
        return {
            'view_mode': 'form',
            'res_id': id_file.id,
