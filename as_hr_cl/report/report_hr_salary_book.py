...
class report_hr_salary_employee_bymonth(models.AbstractModel):
    _name = 'report.as_hr_cl.report_hrsalarybymonth'
    ...
    def get_worked_days(self, form, emp_id, emp_salary, mes, ano):
        ...
        self.env.cr.execute(
            ...
            left join hr_work_entry_type as rw on rw.id = p.work_entry_type_id
            where r.employee_id = %s  and (to_char(date_to,'mm')= %s)
            and (to_char(date_to,'yyyy')= %s) and ('WORK100' = rw.code)
            ''', (emp_id, mes, ano,))
...
group by r.name, p.date_to''', (cod_id, mes, ano,))
    ...
    def get_analytic(self, form):
        ...
        employer_id = form['employer_id'][0]
        cont = 0
...
left join as_hr_employer as ep on ep.id = r.employer_id
left join account_analytic_account as w on w.id = r.analytic_account_id
where p.state = 'done' and (to_char(p.date_to,'mm')=%s)
and (to_char(p.date_to,'yyyy')=%s) and r.employer_id = %s
group by w.name order by name''', (last_month, last_year,employer_id))
...
order by r.analytic_account_id, last_name''', (last_month, last_year,employer_id))
